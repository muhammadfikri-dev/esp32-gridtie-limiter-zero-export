/**
 * =========================================================================================
 * Project     : ESP32 Solar Grid-Tie Zero Export Limiter & Inverter Power Throttle
 * Platform    : ESP32 (Dual-Core Xtensa LX6 / FreeRTOS)
 * Framework   : Arduino IDE 2.0+
 * Author      : Muhammad Fikri
 * License     : MIT
 * Description : Zero-export solar power manager monitoring grid import/export via high-speed CT clamp, throttling grid-tie inverters via Modbus RS485 / SunSpec in under 200ms.
 * =========================================================================================
 */
...
void TaskTelemetryBroadcaster(void *pvParameters) {
    ...
    for (;;) {
        if (xSemaphoreTake(g_telemetryMutex, pdMS_TO_TICKS(20)) == pdTRUE) {
            ...
            doc["system"] = "esp32-gridtie-limiter-zero-export";
...
void setup() {
    ...
    Serial.println(F("================================================================="));
    Serial.println(F(" ESP32 Solar Grid-Tie Zero Export Limiter & Inverter Power Throttle"));
